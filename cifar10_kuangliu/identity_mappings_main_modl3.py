...
def main():
    ...
    if args.scratch == False:
        ...
        ckpt = torch.load('/home/personnels/garg191/baselines/c10_resnet110_3248/checkpoint_final.pth')
        print("loaded checkpoint\n /home/personnels/garg191/baselines/c10_resnet110_3248/checkpoint_final.pth \n")
        new_dict_to_load = {}
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_accuracy, test_loss, test_loss_cls, test_loss_norm = validate(testloader, model, criterion, 0)
        train_accuracy, train_loss, train_loss_cls, train_loss_norm = validate(trainloader, model, criterion, 1)

        info = { 'tot_test_loss': test_loss, 'test_loss_cls': test_loss_cls, 'test_loss_norm': test_loss_norm, 'test_accuracy': test_accuracy, 'tot_train_loss': train_loss, 'train_loss_cls': train_loss_cls, 'train_loss_norm': train_loss_norm, 'train_accuracy': train_accuracy}
...
def validate(testloader, model, criterion, flag=0):
    ...
    with torch.no_grad():
        for i, (inputs, target) in enumerate(testloader):
            ...
            l2_norm = torch.mean(g_validate**2, 0)
            classification_loss = criterion(output, target)
            mem_loss = args.lambda_mem * l2_norm
            loss = classification_loss + mem_loss
    ...
    return acc, test_loss/total, test_loss_cls/total, test_loss_norm/total
...
def save_checkpoint(state, name):
    ...
    """
    path = '/data/chercheurs/garg191/checkpoint/cifar_resnets_modified/' + p + '/c{}_{}_3248_{}'.format(args.dataset[5:], args.arch[:3]+str(args.depth), x)
