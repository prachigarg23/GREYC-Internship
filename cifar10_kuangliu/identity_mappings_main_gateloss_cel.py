...
import models.resnet_ex_1by1element as resnet_ex_1by1element
import models.resnet_ex_1by1tube as resnet_ex_1by1tube
import models.resnet_ex_3by3tube as resnet_ex_3by3tube
...
parser.add_argument('--lambda_mem', default=0.01, type=float,
...
if args.arch.endswith('1by1element') and args.block_name == 'BasicBlock':
    block__ = resnet_ex_1by1element.BasicBlock
elif args.arch.endswith('1by1element') and args.block_name == 'Bottleneck':
    block__ = resnet_ex_1by1element.Bottleneck
elif args.arch.endswith('1by1tube') and args.block_name == 'BasicBlock':
    block__ = resnet_ex_1by1tube.BasicBlock
elif args.arch.endswith('1by1tube') and args.block_name == 'Bottleneck':
    block__ = resnet_ex_1by1tube.Bottleneck
elif args.arch.endswith('3by3tube') and args.block_name == 'BasicBlock':
    block__ = resnet_ex_3by3tube.BasicBlock
elif args.arch.endswith('3by3tube') and args.block_name == 'Bottleneck':
    block__ = resnet_ex_3by3tube.Bottleneck

if args.block_name.lower() == 'basicblock':
    num_layers_arch = (args.depth - 2) // 6
elif args.block_name.lower() == 'bottleneck':
    num_layers_arch = (args.depth - 2) // 9
...
def main():
    ...
    tf_dir = 'runs_c{}_{}_3248_{}'.format(args.dataset[5:], args.arch+str(args.depth),p+x)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_accuracy, train_loss, train_loss_cls, train_loss_cel = validate(trainloader, model, criterion, 1)

        info = { 'tot_test_loss': test_loss, 'test_loss_cls': test_loss_cls, 'test_loss_cel': test_loss_gatecel, 'test_accuracy': test_accuracy, 'tot_train_loss': train_loss, 'train_loss_cls': train_loss_cls, 'train_loss_cel': train_loss_cel, 'train_accuracy': train_accuracy}
...
def train(trainloader, model, criterion, optimizer, epoch):
    ...
    for ind, (inputs, target) in enumerate(trainloader):
        ...
        gate_loss = block__.loss_g
        gate_loss = gate_loss.to(device)

        gate_cel = gate_loss/(num_layers_arch*3) # divide by the total number of residual blocks in the architecture
...
def save_checkpoint(state, name):
    ...
    """
    path = '/data/chercheurs/garg191/checkpoint/cifar_resnets_modified/' + p + '/c{}_{}_3248_{}'.format(args.dataset[5:], args.arch+str(args.depth), x)
