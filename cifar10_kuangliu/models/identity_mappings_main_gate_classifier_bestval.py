...
parser.add_argument('--trainset_split_ratio', default=0.1, type=float,
...
def main():
    ...
    #tf_dir = 'runs_c{}_{}_scit1g-2cm-1_{}'.format(args.dataset[5:], args.arch+str(args.depth), args.mod_name_suffix)
    #writer = SummaryWriter('cifar_resnets_findingbestval/' + tf_dir)
    ...
    split = int(np.floor(args.trainset_split_ratio * num_train)) #using a 45k/5k train val split, i.e. its a 10% split
    train_idx, valid_idx = indices[split:], indices[:split]

    '''
    trainset_new = []
    for i in train_idx:
        trainset_new.append(trainset.__getitem__(i))
    valset_new = []
    for j in valid_idx:
        valset_new.append(valset.__getitem__(j))

    trainloader = torch.utils.data.DataLoader(trainset_new, batch_size=args.batch_size, shuffle=True, num_workers=args.workers)
    validloader = torch.utils.data.DataLoader(valset_new, batch_size=args.batch_size, shuffle=False, num_workers=args.workers)
    '''

    print('\nrunning essential checks, ')
    print('\ntensorboard and checkpoint directory')
    print('args.scratch {},\t args.gate_iters {},\t args.lr_gate {},\t args.schedule_gate {},\t args.trainset_split_ratio {},\t args.schedule {},\t args.epochs {}'.format(args.scratch, args.gate_iters, args.lr_gate, args.schedule_gate, args.trainset_split_ratio, args.schedule, args.epochs))
    # checks - the train_idx, valid_idx are mutually exclusive union = 50000, the trainset_new, valset_new are lists of tuples where each tuple has an image tensor and a target label
    # print('size of trainset_new, valset_new: ', len(trainset_new), len(valset_new))

    # checking what happens by using the SubsetRandomSampler
    train_sampler = SubsetRandomSampler(train_idx)
    valid_sampler = SubsetRandomSampler(valid_idx)
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, sampler=train_sampler, num_workers=args.workers)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        val_accuracy, val_loss_cls, val_loss_gatecel = validate(validloader, model, criterion, 0)
        test_accuracy, test_loss_cls, test_loss_gatecel = validate(testloader, model, criterion, 1)
        train_accuracy, train_loss_cls, train_loss_gatecel = validate(trainloader, model, criterion, 1)

        info = {'test_loss_gatecel': test_loss_gatecel, 'test_loss_cls': test_loss_cls, 'test_accuracy': test_accuracy, 'val_loss_gatecel': val_loss_gatecel, 'val_loss_cls': val_loss_cls, 'val_accuracy': val_accuracy, 'train_loss_cls': train_loss_cls, 'train_loss_gatecel': train_loss_gatecel, 'train_accuracy': train_accuracy, 'lr': optimizer.param_groups[0]['lr'], 'gate_lr': optimizer_gate.param_groups[0]['lr']}
...
def train(trainloader, model, criterion, optimizer, optimizer_gate, poch):
    ...
    for ind, (inputs, target) in enumerate(trainloader):
        ...
        #print("\nTest 1, g_layer weights before gate loss gradient update ", model.module.g_layer[0].weight.mean().item())
        #print("\nTest 1, layer 2 weights before gate loss gradient update ", model.module.layer2[0].conv2.weight.mean().item())
        ...
        #print("\nTest 2, g_layer weights after gate loss gradient update ", model.module.g_layer[0].weight.mean().item())
        #print("\nTest 2, layer 2 weights after gate loss gradient update ", model.module.layer2[0].conv2.weight.mean().item())
        ...
        if (ind+1) % args.gate_iters == 0:
            ...
            #print("\n\nTest 3, g_layer weights after classification loss gradient update ", model.module.g_layer[0].weight.mean().item())
            #print("\nTest 3, layer 2 weights after classification loss gradient update ", model.module.layer2[0].conv2.weight.mean().item())
...
def validate(testloader, model, criterion, flag=0):
    ...
    return acc, test_loss_cls/total, test_loss_gatecel/total
...
def save_checkpoint(state, name):
    ...
    """
    path = '/data/chercheurs/garg191/checkpoint/cifar_resnets_findingbestval/c{}_{}_scit1g-2cm-1_{}'.format(args.dataset[5:], args.arch+str(args.depth), args.mod_name_suffix)
